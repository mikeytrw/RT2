...
#### S7 final acceptance correction (2026-08-22)

The preceding S7 acceptance addendum was a provisional record and its ranges,
counts, and mutant descriptions are superseded by this correction. Commit
`0663038` adds only acceptance tests and the stale ownership/comment cleanup;
no propagation behavior was changed. The tracked fixture was restored from
the reviewer-generated bytes before and after verification.

| Case | Current literal test and evidence |
|---|---|
| A1 | `RT2Tests/src/Phase8W4S7AcceptanceTests.cpp:253-566`, `Phase 8 W4 S7 A1: source edit propagates through prepare stage command and reload`, **137/137**. The chain creates the real textured GLB, three instances and one local retint, edits the durable source, runs Prepare→Stage→`EditorCommandHistory`, checks exact pre/post mesh/material/texture extents and append-slot ownership, saves/reloads/ResolveAll, and checks exact serialized table extents (1 mesh, 13 materials, 1 texture), per-instance slots, transforms, visibility and local precedence. The continuation remains the literal A5 rebind/quarantine/sibling chain. |
| A10 | `RT2Tests/src/Phase8W4S7AcceptanceTests.cpp:567-690`, **32/32**. Execute→undo→redo asserts serialized/deep resource state, affected UUID/`SyncImpact`, append-only residency, history and generation behavior, one fingerprint read, and silent no-op. It does not claim an authoring-revision increment; the earlier revision wording is superseded. |
| A13 | `RT2Tests/src/Phase8W4S7AcceptanceTests.cpp:691-953`, **134/134**. Both primitive→imported and imported→primitive now literally run Prepare→Stage→`EditorCommandHistory::Execute`, assert exclusive provenance, exact primitive/imported payload, staged/rebased MeshRef and resource ownership slots, then Undo/Redo with immutable extents. The same test retains material-conflict quarantine and valid-sibling continuation. |
| A11 | `RT2Tests/src/Phase8W4S7AcceptanceTests.cpp:954-1085`, **38/38**. This case is explicitly host-orchestration evidence (scene-open/recovery, explicit Edit, watcher Edit, queued Play→Edit, counts and zero Play mutation), not a handcrafted claim of cross-route source-byte equivalence. Literal default source-byte→Prepare→Stage evidence remains in S5 `RT2Tests/src/Phase8W4S5LoadIntegrationTests.cpp` and S6 `RT2Tests/src/Phase8W4S6LiveReimportTests.cpp`; this wording narrows the claim rather than inventing product behavior. |

The final S7 filter is **4/4 cases and 341/341 assertions** in both Release
and Debug. The final full baseline is **1,054/1,054 cases and
155,466/155,466 assertions** in both configurations. The W4 focused result is
**67/67 cases and 2,032/2,032 assertions** in both configurations. All-target
Release and Debug solution builds completed with 0 errors; `run_script_test`,
`run_slice_test`, `run_recovery_test`, and direct SliceRunner standalone,
project, and report modes passed in both configurations.

Independent compiling production RED/GREEN probes were restored byte-for-byte:
the A13 dual-provenance removal mutant failed **2 assertions** (including the
Undo/Redo exclusive-state checks); the A13 omitted-MeshRef mutant failed **1
assertion**; and the A1 append/leak mutant failed **7 assertions** in its
literal test. Each production source diff was empty after restoration and the
GREEN focused suite returned to full counts. These are acceptance probes only;
no mutant remains in production.

`graphify update .` was run after the final test edits at the `0663038` code
commit boundary. The regenerated `graphify-out/GRAPH_REPORT.md` records
`Built from commit: 06630385`; the subsequent docs commit does not alter the
code graph. `git diff --check` is clean for authored source/tests/docs; any
generated-report whitespace is emitted by Graphify and is reported separately,
not normalized by hand. The stale debounce ownership comment is corrected at
`RT2App/src/PrefabPropagationLive.h:122-127`, and the S6 policy test title is
current at `RT2Tests/src/Phase8W4S6LiveReimportTests.cpp:733`.
